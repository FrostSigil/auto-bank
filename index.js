...
module.exports = function AutoBank(mod) {
	mod.dispatch.addDefinition("C_PUT_WARE_ITEM", 3, path.join(__dirname, "defs", "C_PUT_WARE_ITEM.3.def"));
	mod.dispatch.addDefinition("S_VIEW_WARE_EX", 2, path.join(__dirname, "defs", "S_VIEW_WARE_EX.2.def"));
	...
	function sViewWareEx(event) {
		...
		if (event.items.length > 0 && mod.game.inventory.items.length > 0) {
			...
			for (const toBank of toBankItems)
				mod.toServer("C_PUT_WARE_ITEM", 3, {
					...
					offset: event.offset,
					fromPocket: toBank.pocket,
					fromSlot: toBank.slot,
					id: toBank.id,
					dbid: toBank.dbid,
		...
		mod.setTimeout(() => {
			if (!Enabled) return;
			if (event.offset + 72 < event.numUnlockedSlots)
